...
// working code - we can get info of network adapters
/*#include <windows.h>
#include <iphlpapi.h>
#include <iostream>
...
void ListNetworkAdapters() {
  ...
  if (dwRetVal == ERROR_BUFFER_OVERFLOW) {
    // Allocate a buffer large enough for the adapter information
    PIP_ADAPTER_INFO adapterInfo = (IP_ADAPTER_INFO *)malloc(dwSize);

    // Now call GetAdaptersInfo again with the allocated buffer
    dwRetVal = GetAdaptersInfo(adapterInfo, &dwSize);
    if (dwRetVal == ERROR_SUCCESS) {
      PIP_ADAPTER_INFO pAdapterInfo = adapterInfo;
      while (pAdapterInfo) {
        std::wcout << L"Adapter Name: " << pAdapterInfo->AdapterName << std::endl;
        std::wcout << L"Adapter Description: " << pAdapterInfo->Description << std::endl;
        std::wcout << L"IP Address: " << pAdapterInfo->IpAddressList.IpAddress.String << std::endl;
        pAdapterInfo = pAdapterInfo->Next;
      }
    } else {
      std::cerr << "GetAdaptersInfo failed with error code " << dwRetVal << std::endl;
    }
...
/*int main() {
  ...
  std::cout << "Number of interfaces found: " << devList.size() << std::endl;
